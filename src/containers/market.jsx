...
const Market = ({
  ...
  const showProducts = () => {
    ...
    const items = filtered_products || products;

    const display = items.map((product, index) => (
      <div key={index}>
        <ProductsCard product={product} />
      </div>
    ));

    const groupedDisplay = display.reduce((result, item, index) => {
    ...
    return groupedDisplay.map((group, index) => (
      <div key={index} className="grid md:grid-cols-3">
        {group}
      </div>
    ));
  };
